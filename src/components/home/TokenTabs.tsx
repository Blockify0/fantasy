...
const TokenTabs = ({ selectedToken, onTokenSelect }: TokenTabsProps) => {
  const tokens = ["ETH", "usdt", "usdc", "card"];
  ...
  return (
    <div className="flex justify-center items-center w-full rounded-[10px]">
      {tokens.map((token, i) => (
        ...
        >
          <img src={`/assets/images/svg-icons/${token}.svg`} alt={token} className="h-[26px]"/>
